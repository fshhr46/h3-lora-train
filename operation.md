...
- **动漫底模 + 社区 LoRA**：`--base noobai`（NoobAI-XL 1.1，SDXL 架构，Danbooru 标签式 prompt，建议 steps 28 / CFG 5–6，负面词 `worst quality, low quality, lowres, bad anatomy, bad hands, extra digits, watermark`）。**单文件 LoRA**（Civitai/kohya `.safetensors`）放在 `/mnt/elements/models/loras/sdxl/`，用 `--lora 文件名 --lora_weight 0.8`（多个逗号分隔；也接受绝对路径或 PEFT 目录）。LoRA 的触发词按作者说明放 prompt 最前。已验证：`reparkz_cknb_v01-000025`（Kitedaio/Reparkz 画风，触发词 `@r3p4rkz,`，配 noobai）。
- **从 Civitai 下载**：需要 API key，已存 furnance `~/.config/civitai_token`。命令：`curl -L -H "Authorization: Bearer $(cat ~/.config/civitai_token)" -o /mnt/elements/models/loras/sdxl/<名字>.safetensors "<Civitai 下载链接>"`；元数据查 `https://civitai.com/api/v1/models/<id>`（看 baseModel、trainedWords、downloadUrl）。LoRA 的 `baseModel` 要和底模系一致（NoobAI/Illustrious 的配 `noobai`，写实的配 `realvis`/`juggernaut`），跨系用会没效果。
