...
# 环境变量: H3_GPU(默认1) H3_PORT(8091) H3_TASK(fl2va) H3_ATTN(CUDNN_ATTN|SDPA|FLASH_ATTN)
set -u
UNIT=h3-server
  ...
  start)
    ...
    systemd-run --user --unit=$UNIT --collect --description="MiniMax H3 (vLLM-Omni) on GPU $H3_GPU :$H3_PORT" \
      ...
      bash -c "source $VENV/bin/activate && exec vllm serve MiniMaxAI/MiniMax-H3 --omni --trust-remote-code \
        ...
        --task-type $H3_TASK --enforce-eager --diffusion-attention-backend ${H3_ATTN:-CUDNN_ATTN} --vae-use-tiling \
        >> $LOG 2>&1"
